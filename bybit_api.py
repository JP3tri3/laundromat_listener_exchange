...
class Bybit_Api:
    ...
    def __init__(self, api_key, api_secret, symbol, symbol_pair, key_input, test_tf):
        self.client = bybit.bybit(test=test_tf, api_key=api_key, api_secret=api_secret)
        self.symbol = symbol
...
#order:
    ...
    # setup to loop through all orders and cancel individually:
...
#orders:
    ...
    def force_limit_order(self, side, input_quantity, limit_price_difference, stop_loss, reduce_only, link_id):
        print('')
        ...
        while(total_pos_size != pos_size):
            ...
            if (new_num_orders == num_orders):
                ...
                if (last_price != current_price) and (last_price != price):
                    ...
                    price = self.calc_limit_price_difference(side, last_price, limit_price_difference)
                    self.change_order_price_size(price, input_quantity, order_id)
                    print(f"Order Price Updated: {price}\n")
            else:
                current_price = self.last_price()
                price = self.calc_limit_price_difference(side, current_price, limit_price_difference)
    ...
    # input order id to change order price & size
...
#Profit & Loss
    ...
    def closed_profit_loss_quantity(self, index):
